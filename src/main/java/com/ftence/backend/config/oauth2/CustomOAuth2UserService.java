...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private User saveOrUpdate(OAuthAttributes attributes) {
        User user = userRepository.findUserByIntraId(attributes.getIntraId())
                .stream().map(entity -> entity.update(attributes.getIntraId(), attributes.getPicture()))
                .findAny().orElse(attributes.toEntity());
